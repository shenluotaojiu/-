...
protected:
	void move_and_collide(int delta)
	{
		...
		if (velocity.y > 0)
		{
			...
			{
				const Platform::CollosionShape& shape = platform.shape;
...
protected:
	...
	bool is_showing_sketch_frame = false;	// 当前是否显示剪影
	bool can_attack = true;
	bool is_jump_effect_visible = false;	// 跳跃效果是否可见
	bool is_land_effect_visible = false;	// 落地效果是否可见
	bool is_cursor_visible = true;			// 玩家光标是否可以显示
	...
	Timer timer_run_effect_generation;	// 角色跑动特效粒子发射定时器
	Timer timer_die_effect_generation;	// 角色死亡特效粒子发射定时器
	Timer timer_cursor_visibility;		// 玩家光标指示器是否可以显示
